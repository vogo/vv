...
| **vv-prd/** | 制品目录(artifact catalog) | 模型字段、流程步骤、API 页、字典枚举 | 项目根 |
...
> 细粒度制品(字段清单、流程逐步、HTTP 页签、字典枚举)由 `vv-prd/` 承载,`specs/` **引用而不复述**,避免双份副本漂移。
...
- **就近引用代码**:能从代码读出的细节,链接到代码或 `vv-prd/`,不在此复述。
