...
- "询问"弹三选一对话框:allow / allow_always / deny;选过 allow_always 的工具进 `session_allowed_tools`,本进程内不再询问。
...
每条 bash 子命令按 bash-risk-tier 分 Safe / Caution / Dangerous / Blocked:
...
- **三动作**:`log`(放行 + 事件)/ `rewrite`(`<vage:untrusted>` quarantine-wrap,内部闭合标记 defang 为 `</vage:_untrusted_>`)/ `block`(替换为 `IsError` 工具结果)。
