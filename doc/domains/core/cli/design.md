...
> HOW。业务行为见 [spec.md](spec.md);实体字段见 [models.md](models.md)。逐步流程与渲染细节见 `vv-prd/procedures/core/cli` 与 `vv-prd/applications/cli`,此处不复述可从代码恢复的逻辑。
...
垂直堆叠单屏,无页面导航;对话框为 modal 覆盖。详见 [001-main-conversation-view](../../../../vv-prd/applications/cli/pages/core/cli/001-main-conversation-view.md)。
...
**判定顺序**(实现于 `vv/cli/permission.go`,完整逻辑见 [procedure-cli-tool-confirmation](../../../../vv-prd/procedures/core/cli/procedure-cli-tool-confirmation.md) Step 1):
...
详见 [002-confirmation-dialog](../../../../vv-prd/applications/cli/pages/core/cli/002-confirmation-dialog.md)、[003-user-question-dialog](../../../../vv-prd/applications/cli/pages/core/cli/003-user-question-dialog.md);实现 `vv/cli/permission.go`、`vv/cli/askuser.go`。
...
- **启动**:与 HTTP 模式共用初始化路径(加载配置、建 LLM client、注册工具、建代理、建路由),再启 Bubble Tea TUI(进 alternate screen,保留用户原终端内容),显示 welcome。`--debug` 在 TUI 模式写文件(`~/.vv/debug-<pid>.log`,`VV_DEBUG_FILE` 可覆盖)而非 tty,避免污染 TUI;`-p` 模式写 stderr。详见 [procedure-cli-startup](../../../../vv-prd/procedures/core/cli/procedure-cli-startup.md)。
- **关闭**:`/exit` 或 idle 下 Ctrl+C 触发;processing 下首次 Ctrl+C 取消当前运行(回 idle),二次才退出(CLISHUT-01)。退出必须**完整恢复终端**(光标、屏幕模式、echo),并优雅取消活动代理运行(CLISHUT-02/03)。详见 [procedure-cli-shutdown](../../../../vv-prd/procedures/core/cli/procedure-cli-shutdown.md)。
